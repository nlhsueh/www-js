...
// Exercise 1
// 普通的加減乘除，利用js提供的運算子進行運算，再利用toFixed()來控制小數的位數，
// 由於toFixed只接受Float因此使用parseFloat將所有的輸入值轉型為小數，而非整數
function plus() {
...
// Exercise 2 
// 根據輸入值判斷是否滿足不同種類三角形的條件，並能夠添加輸入欄位，
// 一次性判斷多筆資料，為了提升體驗，將資料輸入量的極限限制在五筆
// 輸出對齊很麻煩...
...
// Exercise 5
// 利用質數篩法的方式來取得小於輸入值的所有質數，並利用forEach 遍歷，輸出到畫面上
function showPrime() {
...
// Exercise 6 
// 透過 Regex 來判斷輸入的ID和Email是否符合格式，在這邊使用/(\.[a-z]+)?$/來確認Email結尾的.是否有配合自串
// 在輸入完成並離開輸入欄時，會有 icon 即時表示內容是否符合格式
function checkId() {
...
// Exercise 8 
// 利用split & Regex 將常見的標當作分割點，然後用 forEach 輸出分割後的自串
...
// Exercise 9 
// 1970/1/1 為星期四，用 for 迴圈開始累加天數，並判斷閏年，
// 最後在 mod一周的天數，並對應到陣列中的字串
...
// Exercise 17 
// 隨機選擇三個課程名稱(來自MIT CSIE 之必選修課程清單)和名字 (來自網路統計最受歡迎的10個名子)
// 利用 Math.random() 和二維陣列隨機生成分數
// 用 for 迴圈判斷最高分者和平均分數
// 點擊 refresh 會刷新內容
